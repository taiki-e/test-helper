...
#[cold]
#[track_caller]
fn not_run_on_valgrind() -> ! {
    panic!("not run on Valgrind")
}
...
unsafe fn client_req(
    ...
    unsafe {
        ...
        // https://sourceware.org/git/?p=valgrind.git;a=blob;f=configure.ac;t=VALGRIND_3_27_1#l2648
...
const fn client_req_base(a: u8, b: u8) -> u32 {
...
const MAKE_MEM_NOACCESS: u32 = client_req_base(b'M', b'C');
const MAKE_MEM_UNDEFINED: u32 = client_req_base(b'M', b'C') + 1;
const MAKE_MEM_DEFINED: u32 = client_req_base(b'M', b'C') + 2;
const MAKE_MEM_DEFINED_IF_ADDRESSABLE: u32 = client_req_base(b'M', b'C') + 11;

macro_rules! make_mem {
    ($name:ident, $name_aligned:ident, $req:ident) => {
        #[inline(always)]
        #[track_caller]
        pub fn $name<T: ?Sized>(m: &T) {
            let num_bytes = mem::size_of_val(m);
            let addr = m as *const T as *mut c_void;
            let res = unsafe {
                client_req(
                    0,
                    $req,
        ...
        #[inline(always)]
        #[track_caller]
        pub fn $name_aligned<T: ?Sized, const ALIGN: usize>(m: &T) {
            const { assert!(ALIGN >= 2 && ALIGN.is_power_of_two()) }
            let size = mem::size_of_val(m);
            assert!(size != 0 && size < ALIGN);
            let num_bytes = ALIGN;
            let addr = m as *const T as *mut c_void;
            let offset = addr.addr() & (ALIGN - 1);
            assert!(size <= ALIGN - offset);
            let addr = addr.with_addr(addr.addr() & !(ALIGN - 1));
            let res = unsafe {
                client_req(
                    0,
                    $req,
...
make_mem!(make_mem_no_access, make_aligned_mem_no_access, MAKE_MEM_NOACCESS);
make_mem!(make_mem_undefined, make_aligned_mem_undefined, MAKE_MEM_UNDEFINED);
make_mem!(make_mem_defined, make_aligned_mem_defined, MAKE_MEM_DEFINED);
make_mem!(
    make_mem_defined_if_addressable,
    make_aligned_mem_defined_if_addressable,
    MAKE_MEM_DEFINED_IF_ADDRESSABLE
);
